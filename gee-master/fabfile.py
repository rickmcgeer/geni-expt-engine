...
def deploy():
    local('scp -i %s -r *.js run-gee-server.sh start-gee-master.sh views public routes *.json %s@%s:%s' % (keyfile, user, host, remote_dir))

def deploy_file(file):
    local('scp -i %s -r %s %s@%s:%s/%s' % (keyfile, file, user, host, remote_dir, file))
    
def deploy_test():
    local('scp -i %s -r *.js run-gee-server.sh start-gee-master.sh views public routes *.json %s@%s:%s' % (keyfile, user, host, remote_dir_test))
