...
def makeDNSName(aName):
    if (aName.endswith('.gee-project.net')):
        aName = aName[:-len('.gee-project.net')]
    checkName(aName, 'DNSName')
    return aName + '.gee-project.net'

requesthHeader = 'http://www.gee-project.org:9999/rest/add_node'
...
if __name__ == '__main__':
    ...
    parser.add_argument('-nickname', type=str, nargs=1, required=True, help='SSH Nickname for the new node.  Should be shell-friendly as it will be used in command lines')
    parser.add_argument('-siteName', type=str, nargs=1, required=True, help='Site name for the new node.  For documentation purposes only')
    args = parser.parse_args()
    ip = getIP()
    try:
        dnsName = makeDNSName(args.dnsName[0])
        checkName(args.nickname[0], 'sshNickname')
        if ip:
            argString = '?ipAddress=%s&sshNickname=%s&dnsName=%s&siteName=%s' % (ip, args.nickname[0], dnsName, args.siteName[0])
            requestURL  = requestHeader + argString
            resultString = urllib2.urlopen(requestURL).read()
            result = json.loads(resultString)
            if (result['success']):
                print 'Successfully added node at ip address %s with DNS Name %s and ssh nickname %s' % (ip, dnsName, args.nickname)
            else:
