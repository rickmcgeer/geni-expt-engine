...
@task
def deploy():
    local('scp -i %s config.py  *.plcsh %s@%s:%s' % (keyfile, user, host, remote_dir))
    local('scp -i %s config.py %s@%s:%sgee-master/config.py' % (keyfile,  user, host, remote_dir))

@task
def deploy_file(file):
    if (file == "config.py"):
        local('scp -i %s config.py %s@%s:%sconfig.py' % (keyfile, user, host, remote_dir))
        local('scp -i %s config.py %s@%s:%sgee-master/config.py' % (keyfile, user, host, remote_dir))
    else:
...
def test_gee_script(script):
    run('export GEE_CONFIG_FILE=%s/config.py; cd %s; %s' % (remote_dir, remote_dir, script))
